...
class net(pl.LightningModule):
    def __init__(self, **kwargs):
        ...
        num_view = kwargs["num_view"]
        self.views = torch.arange(num_view) * torch.pi * 2 / num_view
    
    def training_step(self, batch, batch_idx):
        ndct, ldct = batch
        p = self.projector.projection(ndct, self.views)
        spct = self.projector.filtered_backprojection(p, self.views)
        model_kwargs = {"x":spct, "y":ndct, "p":p}
